...
//        return resources;
    ...
    private ArrayList<String> solarSystemNames = new ArrayList<>(Arrays.asList("Acamar",
    ...
    public void generateTechLevels() {
        ...
        for (int i = 0; i < MAXSOLARSYSTEM; i++) {
            actualLevels.add(possibleLevels.get((int) (Math.random() * (TechLevel.values().length))));
        }
    }
...
//        actualResources = new Resources[MAXSOLARSYSTEM];
        ...
        for (int i = 0; i < MAXSOLARSYSTEM; i++) {
            actualResources.add(possibleResources.get((int) (Math.random() * (Resources.values().length))));
        }
    }
